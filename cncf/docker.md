...
- 场景
  ...
  - 创建隔离环境用于测试
- 历史
  - 2010年，在美国旧金山成立一家名叫 “dotCloud” 公司,主要提供基于 PaaS 云计算技术服务。具体来说，是和LXC有关的容器技术
  ...
  - Docker 火了之后，dotCloud 公司把公司名字改成 Docker Inc.
  - Docker 于2016年联手谷歌与IBM共同打造了 containerd 项目,2017年被捐赠给云原生计算基金会（CNCF），随后发展出的 containerd CRI 项目能够与Kubernetes 进行交互，也逐步吸引到来自亚马逊、Google、微软以及IBM乃至整个行业的大量创新支持与资源投入
...
- 典型 CS(Client/Server) 架构软件，后台服务运行 dockerd 引擎作为服务器端，终端用户进行交互 docker 客户端。
  - docker 引擎暴露一组 Rest API 供客户端调用，从而完成各种实际功能。
  - 表面上在本机执行各种 docker 命令，实际上使用远程调用形式在服务端完成
- Docker daemon
  ...
  - 方式
    ...
    - unix:path_to_socket
    - fd://socketfd
  - 可以通过设置命令行flag参数 设置安全传输层协议(TLS)的有关参数，保证传输的安全性
...
- `docker stop` 终止一个运行中容器
  ...
  - exit 后 container就终止，不过并没有消失，可以用 `sudo docker ps -a`，可以启动：`sudo docker start container_id`
...
- `docker exec -it mynginx sh`
  - 过 docker diff 命令看到具体的改动
  - 在运行的容器内使用 apt-get update 命令进行更新,exit退出容器

### logs

- `sudo docker logs container_id`
- 默认日志日志驱动 json-file，将来自容器的 stdout 和 stderr 日志都统一以 json 形式存储到本地磁盘。日志存储路径 `/var/lib/docker/containers/<容器 id>/<容器 id>-json.log`
- 不同日志驱动可以将日志重定向到不同地方，以热插拔的方式实现日志不同目的地的输出
  - 驱动
    - local
    - json-file
