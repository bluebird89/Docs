...
- 高层模块不应直接依赖低层模块，应该依赖于彼此间的抽象
- 能隔离变化，使核心业务更稳定
  - 代码是由业务需求驱动出来的，而业务驱动路径一定是从高层（较为稳定的核心业务层）逐渐传递至低层（较为多变的外围支撑层）。
  - 高层不会去了解低层的实现细节，而只会对低层给出需求的定义。依赖倒置就是要明确需求的定义。
- 对需求进行了明确的定义，产出了需求的稳定抽象，基于此抽象的实现，不论如何变化，都不会影响到核心业务的稳定
- 怎么实现依赖倒置：核心业务方定义需求抽象，服务提供方实现需求抽象。
  - 先由业务消费方定义接口，再由服务提供方实现
  - 从最终产出物的角度看，的确是可能每个实现都抽取了一个接口而已。
