...
- 任务
	...
	- 负载均衡 在对上游或后端服务进行请求时，选择合适服务实例接收请求，同时负责处理超时、断路、重试等情况
	- 身份验证和授权 对网络请求进行身份验证、权限验证，以决定是否响应以及如何响应，使用 mTLS 或其他机制对链路进行加密等
	- 链路追踪 对于每个请求，生成详细的统计信息、日志记录和分布式追踪数据，以便操作人员能够理解调用路径并在出现问题时进行调试
	- 边缘代理，比如 Istio 的 Ingress Gateway、基于 Envoy 实现的 Ingress Controller（Contour、Ambassador、Gloo等
...
- Downstream 下游 主机连接到 Envoy，发送请求并获得响应
- Upstream 上游 主机获取来自 Envoy 的连接接请求和响应
- Cluster: 集群
...
- 规则可以是静态的，放在配置文件中的，在启动的时候加载,支持热加载和热重启
- 最好方式是将规则设置为动态的，放在服务发现（Discovery Service）维护
...
- route。有时候多个 cluster 具有类似的功能，但是是不同的版本号，可以通过 route 规则，选择将请求路由到某一个版本号，也即某一个 cluster。
	- 静态 将后端服务端 IP 地址拿到放在配置文件里面
	- 动态 需要配置一个服务发现中心，这个服务发现中心要实现 Envoy 的 API，Envoy 可以主动去服务发现中心拉取转发策略
  ...
  - 通过订阅（subscription）方式来获取资源
    ...
    - gRPC 流式订阅
      - 单资源类型发现: 每个 xDS API 可以单独配置 ApiConfigSource，指向对应的上游管理服务器的集群地址。每个 xDS 资源类型会启动一个独立的双向 gRPC 流，可能对应不同的管理服务器。API 交付方式采用最终一致性
        - 每个 xDS API 都与给定的资源的类型存在 1:1 对应,采用 `type.googleapis.com/<resource type>` 形式，例如 CDS 对应于`type.googleapis.com/envoy.api.v2.Cluster`。在 Envoy 的请求和管理服务器的响应中，都包括了资源类型 URL
