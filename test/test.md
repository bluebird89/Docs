...
- UI层自动化适用场景是做核心功能的回归测试和冒烟测试，所以在实施过程中，要注意不要把所有用例都堆砌在UI层，而是尽可能放到接口测试和单元测试中去做。
...
- 要编写一个好的测试用例
	- 首先需要对业务需求和验收条件（AC）进行深入的分析，并确定业务需求和验收条件的正确性和合理性
	- 然后对其进行测试分析，并完成整体测试用例的设计和编写，其中包括功能测试用例，E2E测试用例，异常测试用例等等。
...
- 用itest，TestLink等系统管理
	- itest 典型的支持执行管理、用例管理和用例编排管理的界面。
	- 需要一个独立服务器来部署和运
- 使用Cucumber，RF，SVN和GIT等代码活文档、自动化测试框架和代码版本工具
	- 可以同时管理自动化测试用例和手动测试用例，并且更容易跟踪测试用例和测试数据的更改。而劣势是需要测试工程师有足够的工程技术能力来实现。
	- 用Cucumber写的一个Demo，左边是集成开发环境中测试用例的管理文件，每个Feature文件就是一套测试用例。
	- 右图是通过Jenkins生成的测试用例活文档（Test Case Living Document），通过它可以统一的展示出手动测试用例和自动化测试用例的测试结果。
- 使用系统活文档
	- 将代码活文档和系统管理结合，通过测试管理系统编写和管理测试用例，然后会自动生成代码模式的测试用例。也可以只编写代码模式的测试用例，然后自动同步到测试管理文档中。自动化测试在持续集成流水线执行，通过流水线进行展示并同步到测试管理系统中。
