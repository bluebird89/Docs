...
- [C10K 瓶颈(同时连接到服务器的客户达到了一万个)](http://www.kegel.com/c10k.html)
	- 进程上下文切换占用大量资源，顶不住巨大压力. NGINX 带着事件循环出来拯救世界
- 一台服务器在单位时间里能处理的请求越多，服务器能力越高，也就是并发处理能力越强
- 基于机械磁盘或SSD的数据库系统，读写速度远慢于内存，因此单纯磁盘介质数据库无法支撑C50K
- 当今 Web 服务和互联网服务，本质上大部分都是 IO 密集型服务
	- 任务大多是和网络连接或读写相关的高耗时(相对 CPU)任务,瓶颈在于尽可能快速完成高并发、多连接下数据读写
  - 多进程 不仅存在频繁调度切换问题，同时还会存在每个进程资源不共享问题，需要额外引入进程间通信机制来解决
  - 多线程 高并发场景的大量 IO 等待会导致多线程被频繁挂起和切换，非常消耗系统资源，同时多线程访问共享资源存在竞争问题
- 思想
  - 关注平衡 达到平衡架构，才可能是高性能、高并发架构。任何性能问题都会由某个点引起。甚至泛指业务需求与复杂度也要平衡
  - 拆分之道：合适事情让合适技术、合适中间件解决。具体 如何横向、纵向拆分还需分析场景
  - 了解业务场景、问题本质&&了解常用场景下解决方案
  - 按照发现问题、分析问题、解决问题思路来看 
- C10M问题
  - 不要让OS内核执行所有繁重任务：将数据包处理、内存管理、处理器调度等任务从内核转移到应用程序高效地完成，让诸如Linux这样 OS 只处理控制层，数据层完全交给应用程序来处理
  - 现在流行 协处理器+CPU 架构
...
- 指标
  ...
  - 每秒查询率 QPS（Query Per Second）：每秒响应请求数 和吞吐量区分的没有这么明显
...
- 改进服务器并发策略:并发策略目的，是让I/O操作和CPU计算尽量重叠进行，一方面让CPU在I/O等待时不要空闲，另一方面让CPU在I/O调度上尽量花最少的时间
...
- 服务层虽然可以分布式扩展，但受限于 DB 连接，并不意味着可以无限扩展。如果是公共服务层不区分秒杀业务和普通业务，是不好做流量过滤的，因为会影响到普通业务的正常流量，这种情况下只能从应用层想办法。如果是单独为秒杀流程服务，或者说流量来源能区分出秒杀业务或其他业务的，那还是有思路的，办法总比困难多
  - 随机数过滤：将一定百分比用户请求直接过滤返回给应用层包装，以友好方式返回提示给用户
...
- 展示层：随机数过滤，将一定百分比的流量请求直接以友好的方式提示给用户。正常讲展示层是不应该过滤的，请求都没有到服务器，但从业务角度看，抢购秒杀本身就是一个概率事件，并不是完全取决于先后顺序 (有时后来的反而能抢到，这取决于分布式服务器处理、网络、排队系统的异步处理等)。虽然对技术人来说欺骗用户的感觉很不耻，但关键时刻偶尔抱一下佛脚也是一种办法，总比系统被冲垮了好。
