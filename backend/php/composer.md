...
- 自动加载可以使用`__autoload()`和`sql_autoload_register()`两种机制
...
- 编辑 composer.json ,增加或更新细节信息，比如库描述、作者、更多参数，甚至仅仅增加一个空格，都会改变文件的md5sum。Composer 会警告哈希值和composer.lock中记载不同:`composer update nothing`

```sh
composer config -l|--list --global
...
- 两种抽象的访问当前命名空间内部元素的方法，`__NAMESPACE__` 魔术常量和namespace关键字
  - 常量 `__NAMESPACE__` 值包含当前命名空间名称的字符串。在全局的，不包括在任何命名空间中的代码，它包含一个空的字符串
...
- zend_lookup_class_ex
  - EG(class_table)
  - spl_autoload_call  spl_autoload_register:将用户 autoload 函数放到SPL_G(autoload_functions)中，且将spl_autoload_call注册到PHP中。
    *`  Composer\Autoload\ClassLoader::loadClass `
    - findFile
      - class map lookup
      - PSR-4 lookup
      - PSR-0 :autoload_namespaces.php
- $prefix不为空的PSR-4加载规则:比如类`A\B\C`，先找`A\B\`对应目录下面的C.php；再找`A\`对应目录下面的`B\C.php` 以此类推
- $prefix为空的PSR-4加载规则:如果找不到，那就在fallbackDirsPsr4下找`A\B\C.php`文件
- $prefix不为空的PSR-0加载规则:PSR-0支持namespace和下划线分隔的类（PEAR-like class name）；这点对一些需要向namespace迁移的旧仓库很有用 对于类`A\B\C`或者`A_B_C`，先找`A\B\`对应目录下面的C.php；再找`A\` 对应目录下面的`B\C.php` 以此类推
- $prefix为空的PSR-0加载规则:如果找不到，直接在prefixesPsr0中找`A\B\C.php`文件 如果还没有找到，在条件允许的状态下，可以到include path中找`A\B\C.php`文件
- composer install 会遍历　lock 源

## 自动加载

- PHP在需要类定义的时候自动调用的一种机制
- include 或 require
- `__autoload` 调用不存在类时会被自动调用,现在基本废弃
  - 一个文件中不允许有多个 `__autoload()`方法，引入文件中也存在`__autoload()`
  - 一个文件中引入多个文件目录
- `bool spl_autoload_register ([ callable $autoload_function [, bool $throw = true [, bool $prepend = false ]]] )` 注册给定函数,作为 `__autoload` 的实现
  - 函数名称
  - 闭包函数
- `spl_autoload_functions ( void )` 获取所有已注册 `__autoload()` 函数:
- `spl_classes` — 返回所有可用的SPL类
- `spl_autoload_unregister` — 注销已注册`__autoload()`函数
...
- psr-4:支持将命名空间映射到路径。命名空间结尾的`\\`不可省略
  ...
  - 注意:对应`name2\space\Foo`类的类文件的路径为`path2/space/Foo.php`，name2不出现在路径中
- PSR-4 vs PSR-0
  - `$student = new \Bpp\Student()`
  - PSR-4 指定的就当作当前命名空间的目录,代码目录结构是： `app/Student.php` 。即目录不需要包含最外层的命名空间 App
    - 加载”`Foo\Bar\Baz`”时，会去寻找“`src\Bar\Baz.php`”文件，对应的会写入到`vendor/composer/autoload_psr4.php` 文件中
  - PSR-0 指定的是当前命名空间的父目录,代码目录结构是： `bpp/Bpp/Student.php` 。即目录包含最外层命名空间 Bpp
    - 目录名称与命名空间层层对应，类名中的下划线_会被转化成目录分隔符，会导致目录结构变得比较深,加载”`Foo\Bar\Baz`”这个class时，会去寻找“`src\Foo\Bar\Baz.php`”这个文件，这个配置会以map的形式写入生成的`vendor/composer/autoload_namespaces.php`中
  - 最大区别:对下划线（underscore)定义不同
...
- 比较操作符指定包范围：>，>=，<，<=，!=
- 空格或者逗号,表示逻辑上的与
- 双竖线||表示逻辑上的或。其中与的优先级会大于或
- 范围（使用连字符）:1.0 - 2.0
- 通配符去定义版本: 1.0.*
- 下一个重要版本操作符使用波浪号~ `~1.2`
- 折音号^：^1.2.3相当于>=1.2.3 <2.0.0
- 镜像：安装包数据（主要是 zip 文件）一般是从 github.com 上下载的，安装包元数据从 packagist.org 上下载
...
- Github允许下载某个git引用的压缩包。为强制使用压缩包，而不是克隆源代码，使用install和update`--prefer-dist`选项
- 考虑修改，源代码优先 `--prefer-source`
- composer remove 更新很多其它版本库
...
- 加载自定义包
  - 建立存放第三方目录
  - composer.json中 `autoload>classmap`增加文件路径
  - `composer dump-autoload`
- 加载没有制作 Composer，以 require 的方式进行加载
- 构建：确定厂商名称和包名，即形如laravel/framework这样，要确保其全局唯一性，在Packagist中不存在
- 结构
  - src：存放组件源代码
  - tests：存放组件测试代码
  - composer.json：Composer配置文件，用于描述组件，声明组件依赖以及自动加载配置等
    - name："vendor/package" 组件厂商名和包名，也是Packagist中的组件名
    - description：简要说明组件
    - keywords：描述属性的关键字
    - "version":"1.0.2"
    - homepage：组件网站URL
    - repositories仓库地址
      - 默认Composer 使用 Packagist 仓库。通过指定仓库地址，可以从任何地方获取包
  ...
  - README.md：关于组件相关信息、使用文档说明、软件许可证等
    - 组件的名称和描述
    - 安装说明
    - 使用说明
    - 测试说明
