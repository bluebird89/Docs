...
* nginx 作为反向代理服务器，可以做负载均衡和静态资源服务器优势
* 后面两台nodejs应用服务器集群
...
* nginx vs node
  ...
  - nginx 采用第二种方法，依然可能会产生负载不完全均衡和惊群问题。nginx是怎么解决的呢：
...
* Configure ETags
* Make Ajax Cacheable
- 建立连接：由于 HTTP 通信是基于 TCP 协议的，所以需要先建立连接才能开始通信，这也是为什么我们在实现 Web 服务器的时候要指定 IP 和端口的原因，客户端可以根据这个 IP 地址和端口号与服务器建立连接（TCP 三次握手）
