...
## [JavaScript](javascirpt/javascript)
...
* 代码自查
  ...
  - Sonarqube:提供发现 发现威胁、Code Smells、Bugs、Vulnerabilities三大特性
  - snyk：轻量级的本地扫描
* 应对XSS攻击
...
* 启用CSP浏览器安全策略
  - “内容安全策略 (CSP) 是一个额外的安全层，用于检测并削弱某些特定类型的攻击，包括跨站脚本 (XSS) 和数据注入攻击等”.通过HTTP头部 Content-Security-Policy或者HTML meta标签定义一系列白名单规则，限制页面上脚本的执行和资源的加载来源，例如不允许执行内联代码（`<script>`块内容，内联事件，内联样式），禁止执行eval() , newFunction() , `setTimeout([string], ...) `和`setInterval([string], ...) `，达到进一步限制页面脚本的目的
...
现在的前端开发，大多数用的是 ES6/7 的语法，然后用 Babel 编译成 ES5/3 后让浏览器运行。 Node 对 ES6/7 的支持并不是十分友好，就算是最新版本的 node，也不还是不支持 ES Module （常看到的 `import` 语法引入模块）。所以要达到同一份代码两端运行的目的，就必须磨平运行环境的差异。
