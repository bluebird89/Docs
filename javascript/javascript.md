...
- 世界上最流行脚本语言。运行在浏览器中的解释型的编程语言
- ECMA-262 标准后被另一个国际标准化组织 ISO（International Organization for Standardization）批准，标准号 ISO-1626
- 灵活语法，表达力强
...
- 使用范围，慢慢超越了浏览器，正在向通用的系统语言发展。各种宿主环境通过这个模型，描述自己的功能和操作接口，从而通过 JavaScript 控制这些功能
...
- 浏览器环境
  - JavaScript 发明目的，作为浏览器内置脚本语言，为网页开发者提供操控浏览器的能力。目前唯一一种通用浏览器脚本语言，所有浏览器都支持。可以让网页呈现各种特殊效果，为用户提供良好的互动体验
  - 浏览器平台化：随着 HTML5 的出现，浏览器本身功能越来越强，不再仅仅能浏览网页，而是越来越像一个平台，JavaScript 因此得以调用许多系统功能，比如操作本地文件、操作图片、调用摄像头和麦克风等等
  - 提供额外 API
    - 浏览器控制类 操作浏览器
    - DOM 类 操作网页的各种元素
    - Web 类 实现互联网的各种功能
- [服务器环境 Node](../backend/nodejs/nodejs)
  - 使得 JavaScript 开发服务器端项目，有些嵌入式平台（Raspberry Pi）能够安装 Node
  - 提供各种操作系统 API
    - 文件操作 API
    - 网络通信 API
- 客户端 JavaScript
...
- 默认 type 是JavaScript，所以不必显式地把 type 指定为 JavaScript
- 由`<script>...</script>`包含的代码就是JavaScript代码，将直接被浏览器执行。JavaScript代码可以直接嵌在网页的任何地方，不过通常我们都把JavaScript代码放到`<body>`后
- 把 JavaScript 代码放到一个单独的.js文件，然后在HTML中通过`<script src="..."></script>`引入这个文件
  - 更利于维护/复用
  - 在同一个页面中引入多个.js文件
...
#### 原始数据类型|基本类型|不可变类型 Primitive data types

##### Boolean

- 布尔值和布尔代数的表示完全一致，一个布尔值只有true、false两种值.常用在条件判断中
- `&&` 与运算 `||`和运算 `!`非运算
...
- 不区分整数和浮点数，统一用Number表示.有时候用十六进制表示整数比较方便，十六进制用0x前缀和0-9，a-f表示,以及运算符
...
#### 对象类型|引用数据类型|可变类型 Object types

- 比较 引用的比较，当且仅当引用同一个对象时才相等
- 赋值
- 浅拷贝，仅仅是赋引用值，而不是对象本身，这样如果涉及到嵌套对象（对象属性值是也是对象），在修改一个对象时，会影响到引用的对象

##### 数组 Array

一组按顺序排列的集合，集合每个值称为元素
...
- 包装对象
  - 用new创建。
  - 看上去和原来的值一模一样，显示出来也是一模一样，但类型已经变为object了！所以，包装对象和原始值用`===`比较会返回false
- typeof 获取对象类型
  - 无法区分 null、Array和通常意义上 object
- 不写 new 情况下，Number()、Boolean和String()被当做普通函数，做类型转换
...
- 用JavaScript请求外域（就是其他网站）的URL
  ...
  - CORS Cross-Origin Resource Sharing：HTML5规范定义的如何跨域访问资源。Origin表示本域，也就是浏览器当前页面的域。当JavaScript向外域（如sina.com）发起请求后，浏览器收到响应后，首先检查Access-Control-Allow-Origin是否包含本域，如果是，则此次跨域请求成功，如果不是，则请求失败，JavaScript将无法获取到响应的任何数据。
...
- 类  对象的类型模板
- 实例 实例是根据类创建的对象
- JavaScript不区分类和实例概念，通过原型（prototype）来实现面向对象编程
- 对象模型是基于原型实现的，特点是简单，缺点是理解起来比传统的类－实例模型要困难，
- 最大缺点 继承实现需要编写大量代码，并且需要正确实现原型链
- 基于原型链 委托机制是原型继承的本质
  - 所有的数据都是对象，JS的根对象是Object.prototype对象，它是一个空对象
  - 要得到一个对象，不是通过实例化类而是找到一个对象作为原型来克隆它
  - 对象会记住它的原型；就JS而言，对象的原型其实是其构造器的原型（new），包含在其隐藏属性`_proto_` 中
...
JavaScript对每个创建的对象都会设置一个原型，指向原型对象。
...
- 要基于 Student扩展出 PrimaryStudent，可以先定义出PrimaryStudent
...
- ES2016 中 Module 机制借鉴自 CommonJS
...
- 不同 JavaScript 文件如果使用相同全局变量，或者定义相同名字顶层函数，都会造成命名冲突
- 把代码全部放入唯一名字空间 MYAPP 中，会大大减少全局变量冲突的可能
- JavaScript 变量作用域实际上 函数内部，在for循环等语句块中是无法定义具有局部作用域变量的。为了解决块级作用域，ES6引入了新的关键字let，用let替代var可以申明一个块级作用域的变量
...
- 相当于匿名函数，并且简化了函数定义。箭头函数有两种格式:
  - 只包含一个表达式，连{ ... }和return都省略掉了
  - 包含多条语句，这时候就不能省略{ ... }和return：
- 参数不是一个，就需要用括号()括起来
- 和匿名函数有个明显的区别：箭头函数内部的this是词法作用域，由上下文确定。箭头函数完全修复了this的指向，this总是指向词法作用域，也就是外层调用者obj.由于this在箭头函数中已经按照词法作用域绑定了，所以，用call()或者apply()调用箭头函数时，无法对this进行绑定，即传入的第一个参数被忽略
- 注意点
...
- ECMAScript 2015 引入了作业队列的概念，Promise 使用了该队列（也在 ES6/ES2015 中引入）。这种方式会尽快地执行异步函数的结果，而不是放在调用堆栈的末尾。
  ...
  - 有个游乐园中过山车的比喻很好：消息队列将你排在队列的后面（在所有其他人的后面），你不得不等待你的回合，而工作队列则是快速通道票，可以在完成上一次乘车后立即乘坐另一趟车。
...
- 异步编程的一种解决方案，提供统一的 API，各种异步操作都可以用同样的方法进行处理
- 定义为最终会变为可用值的代理
...
- 构造函数接受一个函数作为参数，函数两个参数分别是resolve和reject函数
  ...
  - resolve reject 作为回调的占位符，通过调用时then、catch实现
- 创建 promise时，已经开始执行
- 真正实现：通过 lambda 返回Promise封装逻辑
  - 内部实现逻辑
  - catch()方法是.then(null, rejection)或.then(undefined, rejection)的别名，用于指定发生错误时的回调函数
    - 在 catch() 内部引发错误，则可以附加第二个 catch()来处理，依此类推
- 特点
  - 对象状态不受外界影响。Promise对象代表一个异步操作，有三种状态：pending（进行中）、fulfilled（已成功）和rejected（已失败）。只有异步操作的结果，可以决定当前是哪一种状态，任何其他操作都无法改变这个状态。这也是Promise这个名字的由来，它的英语意思就是“承诺”，表示其他手段无法改变。
...
- 实例生成以后，可以用then方法分别指定resolved状态和rejected状态的回调函数
  ...
  - 第二个回调函数（可选）是Promise对象的状态变为rejected时调用
...
- 在很短的时间内从回调发展到了 promise（ES2015），且自 ES2017 以来，异步的 JavaScript 使用 async/await 语法甚至更加简单
- 异步函数是 promise 和生成器的组合，基本上是 promise 的更高级别的抽象。 而 async/await 建立在 promise 之上
- 减少了 promises 的样板，且减少了 promise 链的“不破坏链条”的限制
- 任何函数之前加上 async 关键字意味着该函数会返回 promise
- 工作原理
  - 异步函数会返回 promise
    - 在任何函数之前加上 async 关键字意味着该函数会返回 promise。即使没有显式地这样做，也会在内部使它返回 promise
  - 调用此函数时，则在前面加上 await，然后调用的代码就会停止直到 promise 被解决或被拒绝。 注意：客户端函数必须被定义为 async
- 多个异步函数串联
...
- 更新：更新该DOM节点的内容，相当于更新了该DOM节点表示的HTML的内容
  ...
  - 如果这个DOM节点是空的，例如，`<div></div>`，那么，直接使用`innerHTML = '<span>child</span>'`就可以修改DOM节点的内容，相当于“插入”了新的DOM节点。如果这个DOM节点不是空的，那就不能这么做，因为innerHTML会直接替换掉原来的所有子节点。
...
- 删除：将该节点从HTML中删除，相当于删掉了该DOM节点的内容以及它包含的所有子节点。
  ...
  - 当`<p>First</p>`节点被删除后，parent.children的节点数量已经从2变为了1，索引已经不存在
...
- 诞生于2011年，是现在最流行的JavaScript测试框架之一
- 测试脚本
...
- 支持不同 helper:WebDriver,Puppeteer,Protractor,Nightmare,Testcafe，在项目上选用的是Puppeteer。
- 支持web也支持mobile，当时项目上的第一个产品是有手机端版本的， 这也是选择这个工具的一个考虑
- 封装良好的页面元素操作方法，拿来即用，对于不擅长编码的来说，非常友好.CodeceptJS 提供了现成的codeceptjs-resemblehelper以实现视觉上的回归测试
...
- [aurelia](https://github.com/aurelia/framework) The aurelia framework brings together all the required core aurelia libraries into a ready-to-go application-building platform. <http://aurelia.io/home>
- [zepto](https://github.com/madrobby/zepto) Zepto.js is a minimalist JavaScript library for modern browsers, with a jQuery-compatible API <http://zeptojs.com>
  - 一个轻量级的针对现代高级浏览器的JavaScript库，只针对移动端浏览器编写，因此体积更小、效率更高，更重要的是，它的 API 完全仿照 jQuery，如果你会用jquery，那么你也会用zepto。
  - 设计目的是提供 jQuery 的类似的API，但并不是100%覆盖 jQuery 。Zepto设计的目的是有一个5-10k的通用库（目前最新版，gzip 压缩后只有 9.1k）、下载并快速执行、有一个熟悉通用的API，所以你能把你主要的精力放到应用开发上。
- [stimulusjs](https://github.com/stimulusjs/stimulus) A modest JavaScript framework for the HTML you already have <https://stimulusjs.org/>
...
- [mithril.js](https://github.com/MithrilJS/mithril.js) A Javascript Framework for Building Brilliant Applications <http://mithril.js.orgMithril> is a modern client-side Javascript framework for building Single Page Applications. It's small (< 8kb gzip), fast and provides routing and XHR utilities out of the box.
- [avalon](https://github.com/RubyLouvre/avalon) an elegant efficient express mvvm framework <http://avalonjs.coding.me/>
- [backbone](https://github.com/jashkenas/backbone) Give your JS App some Backbone with Models, Views, Collections, and Events <http://backbonejs.org>
...
- [omi](https://github.com/Tencent/omi) Next generation web framework using web components with omio fallback(IE8+) in tiny js. <http://omijs.org>
...
- [whs.js](https://github.com/WhitestormJS/whs.js) 🚀 🌪 Super-fast 3D framework for Web Applications 🥇 & Games 🎮. Based on Three.js <https://whs.io>
