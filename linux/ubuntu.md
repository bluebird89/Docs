...
- “Ubuntu restricted extras” is the package of several codecs that are not included by default in Ubuntu system.
...
- deb包 Debian，Ubuntu等Linux发行版软件安装包，扩展名为.deb，是类似于rpm的软件包，
- 不推荐使用deb软件包，因为要解决软件包依赖问题，安装也比较麻烦。下载相应deb软件包，使用dpkg命令来安装
...
- 默认使用 systemd-resolved 系统服务接管本机 DNS 查询，默认启动且监听 53 端口
...
用来监控Linux系统状况，比如cpu、内存的使用 `top [-] [d] [p] [q] [c] [C] [S] [s]  [n]`
...
```sh
top - 11:22:51 up  3:17,  1 user,  load average: 1.79, 1.68, 1.51

Tasks: 296 total,   2 running, 294 sleeping,   0 stopped,   0 zombie

%Cpu(s):  4.7 us,  0.8 sy,  0.0 ni, 94.4 id,  0.0 wa,  0.0 hi,  0.1 si,  0.0 st

MiB Mem :  15002.8 total,   1068.7 free,   4381.4 used,   9552.7 buff/cache

MiB Swap:  14304.0 total,  14303.7 free,      0.2 used.   9691.8 avail Mem 
```
...
- 第一行 任务队列信息，同 uptime 命令
  ...
  - 1 user  当前登录用户数
  - load average: 0.00, 0.02, 0.05  系统负载，即任务队列平均长度。分别为 1分钟、5分钟、15分钟前到现在平均值
